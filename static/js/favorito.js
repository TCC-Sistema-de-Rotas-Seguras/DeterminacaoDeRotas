...
function carregarFavorios() {
  var len = banco.favoritos.length;
  for (let i = 0; i < len; i++) {
    criarFavorito(banco.favoritos[i].nome,banco.favoritos[i].endereco)
  }
}

function FormFavorito(){
  ...
  const btnAdicionar = document.getElementById('btn-adicionar');
    ...
    form.addEventListener('submit', function (event) {
      ...
      if (nome && endereco) {
        ...
        adicionarFavoritos(banco, nome,endereco)
...
document.addEventListener('click', function (e) { 
  ...
  if (e.target.classList.contains('origem')) {
    ...
    // Simula a seleção manual
    const service = new google.maps.places.PlacesService(document.createElement('div'));
    service.findPlaceFromQuery({ query: endereco, fields: ['geometry'] }, function(results, status) {
        if (status === google.maps.places.PlacesServiceStatus.OK && results[0]) {
            const location = results[0].geometry.location;
            document.getElementById('origin_coords').value = location.lat() + ',' + location.lng();
